...
//		}
    ...
    public static Object[][] voltaDatai(List<Municipio> municipios) {
        String[][] datai = new String[municipios.size()][5];
        int i = 0;
        for (Municipio municipio : municipios) {
            datai[i] = new String[5];
            datai[i][0] = municipio.getSiglaUF();
            datai[i][1] = municipio.getCodUF();
            datai[i][2] = municipio.getCodMun();
            datai[i][3] = municipio.getMunicipio();
            datai[i][4] = municipio.getPopulacao();
            i++;
        }
        return datai;
    }
